...
void main() {
  runApp(ContentLinkApp());
}

class ContentLinkApp extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'News Link Analyzer',
      home: ContentLinkHomePage(),
      debugShowCheckedModeBanner: false,
    );
  }
}

class ContentLinkHomePage extends StatefulWidget {
  @override
  _ContentLinkHomePageState createState() => _ContentLinkHomePageState();
}

class _ContentLinkHomePageState extends State<ContentLinkHomePage> {
  ...
  Future<void> _analyzeUrl() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://192.168.29.46:5001/analyze_url'), // Change as needed
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'url': userInput}),
      );
                        ...
                        children: [
                          Padding(
                            ...
                            child: Wrap(
                              spacing: 6.0,
                              children: _entities
                                  .map((e) => Chip(label: Text('${e[0]} (${e[1]})'))).toList(),
                            ),
                          ),
                        ],
                      ),
